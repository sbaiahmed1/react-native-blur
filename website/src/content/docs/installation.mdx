...
- **Browser support:** `backdrop-filter` is available in Chrome 76+, Edge 79+, Safari 9+ (via `-webkit-`), and Firefox 103+ — ~97% of global usage. In browsers without it (and while server-side rendering), components gracefully degrade to their semi-transparent tint without blur.
