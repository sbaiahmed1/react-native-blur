...
`BlurView` is the standard, cross-platform blur component. On iOS it uses UIKit's `UIVisualEffectView`; on Android it uses [QmBlurView](https://github.com/QmDeve/QmBlurView) for native blur, with a lightweight overlay fallback on limited devices; on Web it uses CSS `backdrop-filter`.
